...
/**
 * Function to create original liquidation pair deployment TX toast while confirming transaction
 *
 * This toast will then update itself on TX success or fail
 */
export const createDeployLiquidationPairTxToast = (data: DeployLiquidationPairTxToastProps) => {
  toast(<DeployLiquidationPairTxToast {...data} />, { id: data.txHash })
}
  ...
  useEffect(() => {
    if (isSuccess && !!txHash) {
      if (!!addRecentTransaction) {
        ...
        const txDescription = 'Liquidation Pair Deployment'
  ...
  return (
    <>
      <span className='flex items-center gap-2 text-pt-purple-50'>
        <Spinner className='after:border-y-pt-teal' /> Deploying liquidation pair...
      </span>
      <a
  ...
  return (
    <>
      <div className='flex flex-col items-center text-center'>
        ...
        <span className='text-pt-purple-50'>You deployed a liquidation pair on {network}</span>
      </div>
      <a
        href={getBlockExplorerUrl(chainId, txHash, 'tx')}
        target='_blank'
