...
export interface SetClaimerTxToastProps {
  chainId: NETWORK
  txHash: `0x${string}`
...
/**
 * Function to create original claimer setting TX toast while confirming transaction
 *
 * This toast will then update itself on TX success or fail
 */
export const createSetClaimerTxToast = (data: SetClaimerTxToastProps) => {
  toast(<SetClaimerTxToast {...data} />, { id: data.txHash })
}
  ...
  useEffect(() => {
    if (isSuccess && !!txHash) {
      if (!!addRecentTransaction) {
        ...
        const txDescription = 'Set Claimer'
  ...
  return (
    <>
      <span className='flex items-center gap-2 text-pt-purple-50'>
        <Spinner className='after:border-y-pt-teal' /> Setting claimer...
      </span>
      <a
  ...
  return (
    <>
      <div className='flex flex-col items-center text-center'>
        ...
        <span className='text-pt-purple-50'>You set a claimer to a prize vault on {network}</span>
      </div>
      <a
        href={getBlockExplorerUrl(chainId, txHash, 'tx')}
        target='_blank'
